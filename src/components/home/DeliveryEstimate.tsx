...
import { Clock, MapPin, ArrowLeft, Car, Star, Phone, Receipt, AlertCircle, RefreshCw, Shield } from 'lucide-react'
...
export default function DeliveryEstimate({ formData, onBack }: DeliveryEstimateProps) {
    ...
    return (
        ...
        <div className="sticky top-0 z-50 bg-white">
            ...
            <div className="flex items-center justify-between">
                ...
                <h2 className="text-xl font-bold text-gray-800">Delivery Estimate</h2>
                </div>
                {status === "searching" && (
        ...
        <div className="max-w-2xl mx-auto space-y-6" style={{marginTop:10}}>
            ...
            >
                ...
                <p className="text-sm mt-1 text-yellow-600">We couldn't find any drivers in your area. Please try again in a few minutes.</p>
                <button
                    onClick={handleRetry}
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                ...
                <span className="font-medium text-gray-800">{SEARCH_STAGES[currentStage]}</span>
                </div>
            ...
            >
                ...
                <div className="flex-1">
                    <div className="flex items-center justify-between">
                    <div>
                        <h3 className="font-semibold text-lg text-gray-900">{driverDetails.name}</h3>
                        <div className="flex items-center gap-2 mt-1">
                        <div className="flex items-center">
                            <Star className="w-4 h-4 text-yellow-400 fill-current" />
                            <span className="text-sm ml-1 font-medium">{driverDetails.rating}</span>
                        </div>
                        <span className="text-gray-300">•</span>
                        <span className="text-sm text-gray-600">{driverDetails.trips} trips</span>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                        <p className="text-sm font-medium text-gray-900">{driverDetails.vehicleType}</p>
                        <p className="text-sm text-gray-600">{driverDetails.vehicleNumber}</p>
                        </div>
                    </div>
                    </div>
                </div>
            ...
            <div className="space-y-4">
                ...
                {(status === "success" || progress > 25) && (
                    ...
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-xl font-bold text-primary">${fare.total.toFixed(2)}</span>
                    </div>
                    <div className="space-y-3 text-sm">
                        <div className="flex justify-between items-center py-2 border-b border-dashed border-gray-200">
                        <span className="text-gray-600">Base Fare</span>
                        <span className="font-medium text-gray-900">${fare.base.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between items-center py-2 border-b border-dashed border-gray-200">
                        <span className="text-gray-600">Distance ({distance})</span>
                        <span className="font-medium text-gray-900">${fare.distance.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between items-center py-2">
                        <span className="text-gray-600">Time</span>
                        <span className="font-medium text-gray-900">${fare.time.toFixed(2)}</span>
                        </div>
                    </div>
                    </motion.div>
                )}
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div className="relative space-y-6">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-600">{formData.dropoff}</p>
                            </div>
                            </div>
                        </div>
                        </div>
            ...
            >
                ...
                <span className="text-sm">Your delivery is insured and tracked in real-time</span>
            </motion.div>
            )}
